...
(* For a simpler interface, we assume that chosing and applying
   ...
   This further assumes that a specification rule is defined
   with a separate precondition that checks the most basic
   shaped of the code, and these can be identified just
   by finding an equation of the right type (conventionally "k").
